...
df4 <- 
  ...
  filter(Final < 2021) %>% 
  arrange(Name) %>%
  select(-Birthday, -Active, -School, -Position, -Debut)
...
```{r}
class_ratio <- nrow(training[training$HOF == 0,]) / nrow(training[training$HOF == 1,])
weights <- ifelse(training$HOF == "1", class_ratio, 1)

# Fit the initial tree with small cp to allow full growth
tree_full <- rpart(HOF ~ ., data = training, method = "class",
                   weights = weights,
                   control = rpart.control(cp = 0.001))  # very small cp
...
rpart.plot(tree_pruned, type = 2, extra = 106)

# Predict and evaluate
tree_preds_pruned <- predict(tree_pruned, testing, type = "class")
confusionMatrix(tree_preds_pruned, testing$HOF, positive = "1")
```


```{r}
df3$HOF_Prob <- predict(tree_pruned, df3, type = "prob")[, "1"]

df3_ind_probs <- df3 %>% select(Name, HOF_Prob, WS, PTS, PER, G, Final) %>%
  arrange(desc(HOF_Prob)) %>%
  gt() %>%
  tab_header(title = "Top 20 Players Most Likely to Make the Hall of Fame")
