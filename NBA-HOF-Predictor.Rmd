...
#CONFUSION MATRIX
conf_mat_rpart <- confusionMatrix(tree_preds, testing$HOF, positive = "HOF")
print(conf_mat_rpart)
...
#Final summary
cat("----- RPART Model Evaluation (Imbalanced Classes) -----\n")
...
grid_tune <- expand.grid(
  nrounds = c(500,1000),
  max_depth = c(4,6),   #We don't want trees to be massive 
  eta = c(0.1),
  gamma = c(1),  # pruning --> Should be tuned. i.e c(0, .05, .1. 0.5, 0.7, 0.9, 1.0)
...
weights <- ifelse(training[,2] == 1,
                  sum(training[,2]==0) / sum(training[,2]==1),
                  1
)

xgb_tune <- train(x = training[,-2],
                  y = training[,2],
...
```{r}
#Now, making the best model based on results
...
predict(xgb_model, testing)
```

```{r}
#Getting evaluation statistics

xgb_preds <- predict(xgb_model, testing) #prediction
xgb_probs <- predict(xgb_model, testing, type = "prob")[, "HOF"]

#Confusion Matrix
conf_mat_xgb <- confusionMatrix(xgb_preds, testing$HOF, positive = "HOF")

#Important Metrics
precision <- conf_mat_xgb$byClass["Precision"]
recall <- conf_mat_xgb$byClass["Recall"]
f1 <- conf_mat_xgb$byClass["F1"]

#ROC CURVE
roc_obj_xgb <- roc(testing$HOF, xgb_probs, levels = c("NotHOF", "HOF"))
plot(roc_obj_xgb)
auc(roc_obj_xgb)
...
#Final summary
cat("----- XGBoost Model Evaluation (Imbalanced Classes) -----\n")
cat("Precision: ", round(precision, 3), "     (How many predicted HOFs were correct?)", "\n")
cat("Recall:    ", round(recall, 3), "     (How many actual HOFs were correctly found?) \n")
cat("F1 Score   ", round(f1, 3),     "     (Balance of precision and recall) \n\n")
cat("Baseline accuracy: ", mean(testing$HOF == "HOF"), "\n\n")
cat("ROC AUC: ", round(auc(roc_obj_xgb), 3), " (Overall discriminative ability - prone to imbalance) \n")
cat("PR AUC:  ", round(pr$auc.integral, 3), "  (Precision-Recall tradeoff - great for imbalance, compare to baseline accuracy)\n")

# Create scores for PR AUC
fg <- xgb_probs[testing$HOF == "HOF"]
bg <- xgb_probs[testing$HOF == "NotHOF"]

#PR CURVE AND PR AUC - XGBOOST
pr <- pr.curve(scores.class0 = fg, scores.class1 = bg, curve = TRUE)
